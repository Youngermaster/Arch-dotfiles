...
title="Youngermaster's rice setup script"
...
downloadDependencies() {
    if grep "Arch\|Artix\|EndeavourOS\|Manjaro" /etc/*-release; then
        ...
        if [[ -e /usr/bin/paru ]]; then
            ...
            paru -S bspwm sxhkd rofi polybar neovim alacritty dunst picom brightnessctl playerctl feh maim viewnior jq xclip imagemagick bsp-layout i3lock-color
        elif [[ -e /usr/bin/yay ]]; then
            echo -e "$prefix yay detected. Installing dependencies..."
            yay -S bspwm sxhkd rofi polybar neovim alacritty dunst picom brightnessctl playerctl feh maim viewnior jq xclip imagemagick bsp-layout i3lock-color
        else
...
copyFiles() {
    ...
    if [ -d $HOME/Pictures/walls/personal ]; then
        cp -r ./etc/walls/personal/personal-5.jpg $HOME/Pictures/walls 
    else 
        mkdir $HOME/Pictures/walls/personal && cp -r ./etc/walls/personal/personal-5.jpg $HOME/Pictures/walls/personal
    fi
